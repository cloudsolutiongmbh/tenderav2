...
## 🔴 TOP 5 PRODUCTION BLOCKER (sofort fixen!)

### 1. ⚠️ AUTH-BYPASS via Environment Variable (CRITICAL - Security)

**Datei:** `packages/backend/convex/auth.ts:13-29`
...
Wenn diese Variable versehentlich in Production gesetzt ist, ist **JEDE API ungeschützt**. Kompletter Security-Bypass.

**Impact:**
- Unauthenticated Access zu allen Ausschreibungen, Angeboten, internen Daten
- Cross-Org Access möglich mit manipuliertem `TEST_ORG_ID`
- Kompletter Data Breach
...
**Priorität:** 🔴 SOFORT - Security-Audit des Deployments durchführen!

---

### 2. ⚠️ Kryptographisch unsichere Token-Generierung (CRITICAL - Security)

**Datei:** `packages/backend/convex/shares.ts:147-156`
...
Fallback verwendet `Math.random()` → Share-Links sind **vorhersagbar und erratbar**.

**Impact:**
- Angreifer kann Share-Links zu fremden Projekten erraten
- Zugriff auf vertrauliche Ausschreibungsunterlagen
- Violiert Vertraulichkeit
...
function fillRandomBytes(bytes: Uint8Array) {
    ...
    throw new Error("Kryptographisch sichere Zufallszahlen nicht verfügbar - Token-Generierung abgebrochen.");
}
```
...
### 3. ⚠️ Infinite Loop + Race Condition bei Share Token Creation (CRITICAL)

**Datei:** `packages/backend/convex/shares.ts:27-36`

**Problem:**
```typescript
let token: string;
while (true) {  // ❌ Kann endlos laufen!
...
**Issues:**
- Keine Timeout-Protection → API kann unbegrenzt lange laufen
- Race Condition: Zwei gleichzeitige Requests können denselben Token generieren
- DoS-Vektor durch absichtlich viele gleichzeitige Requests

**Impact:**
- API-Freeze bei vielen gleichzeitigen Share-Erstellungen
- Potential für duplicate token inserts → Data corruption
- Server-Freeze
